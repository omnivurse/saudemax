/*
  ...
  1. Problem
    - RLS policies using auth.uid() directly are causing performance issues
    - Function is being re-evaluated for each row, which is inefficient
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'Fixed auth.uid() calls in RLS policies:';
  RAISE NOTICE '✅ Replaced direct auth.uid() calls with (SELECT auth.uid())';
  RAISE NOTICE '✅ This ensures the function is only evaluated once per query';
  RAISE NOTICE '✅ Improves performance for tables with many rows';
  RAISE NOTICE '';
