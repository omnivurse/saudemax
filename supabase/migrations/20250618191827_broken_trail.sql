/*
  # Create Qloud Agent User

  1. Purpose
    - Create a demo agent user named "Qloud" for testing
    - Add sample data for the agent dashboard
    
  2. Features
    - Creates a user with agent role
    - Creates affiliate profile
    - Adds sample affiliate links, commissions, and referrals
    
  3. Security
    - Checks for existing auth user first
    - Maintains proper foreign key relationships
*/

-- Fix the users_role_check constraint to include 'agent' role if needed
...
BEGIN
    ...
    SELECT EXISTS(SELECT 1 FROM auth.users WHERE email = 'qloud.agent@saudemax.com') INTO auth_user_exists;
    
    -- Only proceed if the auth user exists
    IF auth_user_exists THEN
        -- Get the actual user ID from auth.users
        SELECT id INTO demo_user_id FROM auth.users WHERE email = 'qloud.agent@saudemax.com';
        
        -- Check if user already exists in public.users
        SELECT EXISTS(SELECT 1 FROM public.users WHERE email = 'qloud.agent@saudemax.com') INTO user_exists;
        ...
        IF NOT user_exists THEN
            ...
            INSERT INTO public.affiliates (
                ...
                demo_user_id,
                'QLOUD-AGENT',
                'https://saudemax.com?ref=QLOUD-AGENT',
                'qloud.agent@saudemax.com',
                'active',
                15.00,
                3250.00,
            ...
            (
                demo_user_id,
                'Facebook Campaign',
                'https://saudemax.com?ref=QLOUD-AGENT&source=facebook',
                now()
            ),
            (
                demo_user_id,
                'Email Newsletter',
                'https://saudemax.com?ref=QLOUD-AGENT&source=email',
                now() - interval '5 days'
            ),
            (
                demo_user_id,
                'Instagram Bio',
                'https://saudemax.com?ref=QLOUD-AGENT&source=instagram',
            ...
            RAISE NOTICE 'Qloud agent user created successfully with ID: %', demo_user_id;
        ELSE
            RAISE NOTICE 'User already exists in public.users table with ID: %', demo_user_id;
        END IF;
    ELSE
        RAISE NOTICE 'Auth user does not exist. Please create qloud.agent@saudemax.com in Supabase Auth first.';
    END IF;
END $$;
...
DO $$
BEGIN
  RAISE NOTICE '=== QLOUD AGENT USER SETUP ===';
  RAISE NOTICE '';
  RAISE NOTICE 'IMPORTANT: You need to create this user in Supabase Auth first:';
  RAISE NOTICE 'Email: qloud.agent@saudemax.com';
  RAISE NOTICE 'Password: Qloud123!';
  RAISE NOTICE 'User metadata: {"role": "agent", "full_name": "Qloud Agent"}';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '     "full_name": "Qloud Agent"';
  RAISE NOTICE '   }';
  RAISE NOTICE '';
