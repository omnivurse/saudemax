/*
  # Populate Roles Table for Existing Users

  1. Purpose
    - Ensure all existing users in auth.users have corresponding entries in the roles table
    - Fix missing role assignments that could cause RLS policy failures
    
  2. Process
    - Identify users in auth.users without entries in the roles table
    - Create role entries based on user_metadata or defaults
...
DROP FUNCTION IF EXISTS populate_missing_roles();
DROP FUNCTION IF EXISTS ensure_users_table_entries();