/*
  ...
  2. Security
    ...
    - Add policy for affiliates to view their own metrics
    - Add policy for admins to view all metrics
*/
...
CREATE POLICY "Affiliates can view their own metrics"
  ...
  FOR SELECT
  TO authenticated
  USING ((affiliate_id = uid()) OR is_affiliate(uid()) OR has_role('admin'));
