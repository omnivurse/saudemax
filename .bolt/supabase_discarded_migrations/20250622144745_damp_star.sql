-- Step 1: Fix the users_role_check constraint to include both 'agent' and 'affiliate' roles
DO $$
BEGIN
  IF EXISTS (
...
-- Step 2: Update any 'agent' roles to 'affiliate' in the users table
UPDATE public.users
SET role = 'affiliate'
WHERE role = 'agent';
...
BEGIN
  ...
  RETURN (
    SELECT (raw_app_metadata->>'role')::text = required_role
    FROM auth.users
    WHERE id = auth.uid()
  );
END;
